...
# 2.2.8
- fix furnace speed increasing with every config save
- fix oil rig npc configurations not working with new scientists
- add separate configuration for large oil rig npcs
- add configs for deep sea npc, patrol boat npc, patrol boat crate
- group npc configs
- add validation for item amount >0
- fix NRE when loading the plugin for the first time

# 2.2.9
- play effect when item is unwrapped with custom loot configuration
- remove item amount validation from excavator and quarry configs (not needed)
- add default loot for new scientists and deep sea patrol boats
- fully custom smelting implementation
- create individual configuration for deep sea RHIB (previously included in PTBoat)

# 2.2.10
- add debugging to OnCollectiblePickup
- fix new npcs not properly destroyed when using custom loot

# 2.2.11
- fix rare NRE in OnCollectiblePickup
- misc

# 2.2.12
- add give button to custom item list
- fix config option "Allow stacking of items with different name"
- add config option "Allow stacking of damaged items with the same condition"

# 2.2.13
- add mission loot box
...
# 2.2.15
- add speed multipliers for new industrial furnaces

# 2.2.16
- added the option for json based loot configs

# 2.2.17
- fix for rust update

# 2.2.18
- fix compile error on oxide without publicizer

# 2.2.19
- add satellite crates
- add metal detector
...
{
    [Info(nameof(Loottable), "The_Kiiiing", "2.2.19")]
    internal class Loottable : BasePlugin<Loottable, Loottable.Configuration>
    {
        ...
        [Hook] object OnLootSpawn(LootFill lootFill)
        {
            var container = lootFill.StorageContainer;
            var entity = lootFill.gameObject.ToBaseEntity();

            if (!container.IsValid() || !entity.IsValid())
            {
                return null;
            }
            ...
            if (config != null && config.Enabled)
            {
                LootManager.PopulateContainer(container, config, lootFill);
                return true;
            }
        ...
        [Hook] object OnItemUnwrap(Item item, BasePlayer player, ItemModUnwrap unwrap)
        {
            if (item?.info != null)
            {
                ...
                if (config != null && config.Enabled)
                {
                    ...
                    {
                        Effect.server.Run(unwrap.successEffect.resourcePath, player.eyes.position);
                    }
        ...
        [Hook] object OnMetalDetectorFlag(MetalDetectorFlag flag, BasePlayer player)
        {
            var config = manager.GetConfig(flag);
            if (config != null && config.Enabled)
            {
                if (flag.Collision != null)
                {
                    flag.Collision.enabled = false;
                }

                LootManager.DropItems(flag, player, config);
        ...
        private static class MetalDetectorFlag_OnFullyDug
        {
            public static bool Prefix(BasePlayer player, MetalDetectorFlag __instance)
            {
                if (Instance?.OnMetalDetectorFlag(__instance, player) != null)
                {
                    return false; // skip
                }
        ...
        [Hook] bool? CanStackItem(Item self, Item other)
        {
            ...
            // Most common checks here
            if (   self.info.itemid != other.info.itemid
                || self.skin != other.skin
                || other.info.stackable <= 1
                || self.MaxStackable() <= 1
                || self == other)
            {
                return false;
            ...
            if ((self.GetOwnerPlayer()?.IsNpc ?? false) || (other.GetOwnerPlayer()?.IsNpc ?? false))
            {
                return null;
            }

            if (   !Config.StackingOptions.IgnoreFullCondition && (!HasMaxCondition(self) || !HasMaxCondition(other))
                || self.hasCondition && !Mathf.Approximately(self.condition, other.condition)
                || !self.IsValid()
                || self.blueprintTarget != other.blueprintTarget
                || !Config.StackingOptions.IgnoreNames && (!String.IsNullOrEmpty(self.name) || !String.IsNullOrEmpty(other.name)) && self.name != other.name
                || (!Config.StackingOptions.IgnoreText || ItemHelper.HasText(self.info.itemid)) && self.text != other.text
                || self.iconImageId != other.iconImageId
                || (self.info.amountType == ItemDefinition.AmountType.Genetics || other.info.amountType == ItemDefinition.AmountType.Genetics) && (other.instanceData?.dataInt ?? -1) != (self.instanceData?.dataInt ?? -1)
                || other.instanceData != null && self.instanceData != null && (other.IsOn() != self.IsOn() || other.instanceData.dataInt != self.instanceData.dataInt && other.info.Blueprint != null && other.info.Blueprint.GetWorkbenchLevel() == 3)
                || HasSignData(self)
                || HasSignData(other)
                || global::Item.BlockStackFoodItem(other, self))
            {
                return false;
            }

            if (ItemHelper.IsLiquidContainer(self.info.itemid) && ((other.contents != null && !other.contents.IsEmpty()) || (self.contents != null && !self.contents.IsEmpty())))
            {
                return false;
            }

            // Only allow stacking of items with same content capacity
            if (other.contents?.capacity != self.contents?.capacity)
            {
                return false;
            }

            if (stackSizeController.LimitHotBarStackSize && IsInHotBar(self) && !stackSizeController.IsVanillaStackable(self))
            {
                return false;
            }
            ...
            static bool HasMaxCondition(Item item)
            {
                // ReSharper disable once CompareOfFloatsByEqualityOperator
                return !item.hasCondition || item.condition == item.info.condition.max;
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static bool HasSignData(Item item)
            {
                return item.instanceData != null && item.instanceData.subEntity.IsValid && ItemHelper.IsSign(item.info.itemid);
            }
        }
        ...
        private static bool CanPopulateContainerHook(StorageContainer container)
        {
        ...
        private void UpdateConfigSubscriptions(bool unsubscribe = false)
        {
            ...
            // Always unpatch to prevent double patches
            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Unpatch(AccessTools.Method(typeof(BaseOven), "Cook"), \u0048\u0061\u0072\u006D\u006F\u006E\u0079PatchType.Prefix, \u0048\u0061\u0072\u006D\u006F\u006E\u0079Id);
            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Unpatch(AccessTools.Method(typeof(BaseOven), "GetSmeltingSpeed"), \u0048\u0061\u0072\u006D\u006F\u006E\u0079PatchType.Postfix, \u0048\u0061\u0072\u006D\u006F\u006E\u0079Id);
            if (!unsubscribe && configurations.Smelting.Enabled && SimpleSplitter == null)
            {
                \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Patch(AccessTools.Method(typeof(BaseOven), "Cook"),
                    prefix: AccessTools.Method(typeof(BaseOven_Patches), nameof(BaseOven_Patches.Cook_Prefix)));
                if (Config.SmeltingOptions.AdjustProgressIndicator)
                {
                    \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Patch(AccessTools.Method(typeof(BaseOven), "GetSmeltingSpeed"),
                        postfix: AccessTools.Method(typeof(BaseOven_Patches), nameof(BaseOven_Patches.GetSmeltingSpeed_Postfix)));
                }
        ...
        object Loottable_OnOvenCook(BaseOven oven, float deltaTime)
        {
            var multiplier = configurations.Smelting?.GetSpeedMultiplier(oven);
            if (!multiplier.HasValue)
            {
                return null;
            }

            SmeltingConfiguration.Cook2(oven, deltaTime, multiplier.Value);

            return true;
        }

        // This is only for visuals
        float? Loottable_OnOvenSpeed(BaseOven oven)
        {
            var speed = configurations.Smelting?.GetSpeedMultiplier(oven);
            if (speed > Config.SmeltingOptions.MaxAdjustValue)
            {
                return null;
            }
        ...
        static class BaseOven_Patches
        {
            public static bool Cook_Prefix(BaseOven __instance, float deltaTime)
            {
                return Instance?.Loottable_OnOvenCook(__instance, deltaTime) == null;
            }

            public static void GetSmeltingSpeed_Postfix(BaseOven __instance, ref float __result)
            {
                var speed = Instance?.Loottable_OnOvenSpeed(__instance);
                if (speed.HasValue)
                {
                    __result *= speed.Value;
                }
            }
        }
        ...
        private void Subscribe()
        {
            ...
            //OnCorpseApplyLoot2
            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Patch(AccessTools.Method(typeof(State_Dead), "StartRagdoll"),
                transpiler: AccessTools.Method(typeof(State_Dead_StartRagdoll), nameof(State_Dead_StartRagdoll.Transpiler)));

            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Patch(AccessTools.Method(typeof(MetalDetectorFlag), "OnFullyDug"),
                prefix: AccessTools.Method(typeof(MetalDetectorFlag_OnFullyDug), nameof(MetalDetectorFlag_OnFullyDug.Prefix)));
        ...
        private void Unsubscribe()
        {
            ...
            //OnCorpseApplyLoot2
            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Unpatch(AccessTools.Method(typeof(State_Dead), "StartRagdoll"), \u0048\u0061\u0072\u006D\u006F\u006E\u0079PatchType.Transpiler, \u0048\u0061\u0072\u006D\u006F\u006E\u0079Id);
            //OnCorpseApplyLoot
            // ReSharper disable once RedundantNameQualifier
            ...
            \u0048\u0061\u0072\u006D\u006F\u006E\u0079Instance.Unpatch(AccessTools.Method(typeof(MetalDetectorFlag), "OnFullyDug"), \u0048\u0061\u0072\u006D\u006F\u006E\u0079PatchType.Prefix, \u0048\u0061\u0072\u006D\u006F\u006E\u0079Id);
        ...
        {
            // Always remove harmony patches to prevent duplicates
        ...
        static class State_Dead_StartRagdoll
        {
            // Credits: ChatGPT
            public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
            {
                ...
                var ownerField = AccessTools.Field(typeof(FSMStateBase), nameof(FSMStateBase.Owner));
                var myHookMethod = AccessTools.Method(typeof(State_Dead_StartRagdoll), nameof(Hook));
                var takeChildrenMethod = AccessTools.Method(typeof(BaseCorpse), nameof(BaseCorpse.TakeChildren));

                // -------------------------------------------------
                // 1️⃣ Find LootableCorpse local via ldloca before Is<T>
                // -------------------------------------------------

                int lootLocalIndex = -1;
                ...
                for (int i = 0; i < codes.Count; i++)
                {
                    if (codes[i].opcode == OpCodes.Call &&
                        ...
                        m.IsGenericMethod)
                    {
                        // The instruction before call is ldloca.s castedUnityObject
                        var prev = codes[i - 1];

                        if (prev.opcode == OpCodes.Ldloca_S || prev.opcode == OpCodes.Ldloca)
                        {
                            if (prev.operand is LocalBuilder lb)
                            {
                                lootLocalIndex = lb.LocalIndex;
                            }
                            else
                            {
                                lootLocalIndex = (int)prev.operand;
                            }
                        }
                ...
                if (lootLocalIndex == -1)
                {
                    throw new Exception("Failed to determine LootableCorpse local index.");
                }

                // -------------------------------------------------
                // 3️⃣ Insert after BaseCorpse.TakeChildren (corpse branch)
                // -------------------------------------------------

                int insertIndex = -1;
                Label branchLabel = default;

                for (int i = 0; i < codes.Count; i++)
                {
                    if (codes[i].Calls(takeChildrenMethod))
                    {
                        insertIndex = i + 1;
                    }

                    if (insertIndex > 0 && codes[i].Branches(out var label) && label.HasValue)
                    {
                        branchLabel = label.Value;
                        break;
                    }
                }
                ...
                if (insertIndex == -1)
                {
                    throw new Exception("Failed to find TakeChildren call.");
                }
                if (branchLabel == default)
                {
                    throw new Exception("Failed to find branch label.");
                }
                ...
                var injected = new List<CodeInstruction>
                {
                    ...
                    new(OpCodes.Ldloc, lootLocalIndex),
                    new(OpCodes.Call, myHookMethod),
                    new(OpCodes.Brtrue, branchLabel)
                };
            ...
            public static bool Hook(State_Dead instance, BaseEntity owner, LootableCorpse corpse)
            {
                ...
                if (owner is ScientistNPC2 npc && corpse != null)
                {
        ...
        static class LootManager
        {
            ...
            public static void DropItems(MetalDetectorFlag flag, BasePlayer player, LootConfig config)
            {
                ...
                foreach (var lootItem in builder.GetAllItems())
                {
                    ...
                    if (item == null)
                    {
                        LogError($"Failed to create item for metal detector loot: {lootItem.ItemId}");
                        continue;
                    }

                    item.AddItemOwnership(player, ItemOwnershipPhrases.MetalDetector);

                    var drop = item.Drop(flag.transform.position + flag.DropOffset, flag.DropVelocity) as DroppedItem;
                    if (drop != null)
                    {
                        drop.NeverCombine = true;
                        drop.SetAngularVelocity(new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f)) * 720f);
                    }
                }
            ...
            public static void PopulateContainer(StorageContainer container, LootConfig lootConfig, [CanBeNull] LootFill lootFill = null)
            {
                if (container == null || !lootConfig.Enabled)
                {
                    return;
                ...
                if (lootConfig.LootType != LootType.Custom && lootFill != null)
                {
                    ApplyVanillaLoot();
                }
                ...
                {
                    if (lootFill.LootSpawnSlots.Length != 0)
                    {
                        foreach (var lootSpawnSlot in lootFill.LootSpawnSlots)
                        {
                            if (lootSpawnSlot.eras == null || lootSpawnSlot.eras.Length == 0 || Array.IndexOf(lootSpawnSlot.eras, ConVar.Server.Era) != -1)
                            {
                                ...
                                {
                                    if (Random.Range(0.0f, 1f) <= (double) lootSpawnSlot.probability)
                                    {
                    ...
                    {
                        if (!(lootFill.LootDefinition != null))
                        {
                            return;
                        }
                        for (int index = 0; index < lootFill.MaxDefinitionsToSpawn; ++index)
                        {
                            lootFill.LootDefinition.SpawnIntoContainer(container.inventory);
                        }
                    }
                }
            }

            public static void PopulateCorpse2(LootableCorpse corpse, LootConfig config, State_Dead state)
            {
                if (!config.Enabled)
                {
                    return;
                ...
                void ApplyVanillaLoot()
                {
                    foreach (var lootSpawnSlot in state.GetField<LootContainer.LootSpawnSlot[]>("LootSpawnSlots"))
                    {
                        for (int index = 0; index < lootSpawnSlot.numberToSpawn; ++index)
                        {
                            if (Random.Range(0.0f, 1f) <= lootSpawnSlot.probability)
                            {
            ...
            public static Item GiveItemToPlayer(LootItem lootItem, BasePlayer player, BaseEntity.GiveItemReason reason = BaseEntity.GiveItemReason.Generic)
            {
                var item = CreateItem(lootItem);
                if (item != null)
                {
        ...
        class LootableManager : ILootableManager
        {
            ...
            public void Init(LootableDataList list)
            {
                ...
                foreach (var data in list.MetalDetectorFlags)
                {
                    AddToLookup(new MetalDetectorFlagLootable(data));
                }
        ...
        private class MetalDetectorFlagLootable : BaseLootable
        {
            //public override LootConfigFlags ConfigFlags =>

            public override bool HasDefaultConfig => true;

            public readonly uint PrefabId;

            public MetalDetectorFlagLootable(MetalDetectorFlagLootableData data)
                : base(data)
            {
                PrefabId = StringPool.Get(data.PrefabName);
            }

            public override IEnumerable<uint> GetPrefabIds()
            {
            ...
            {
                var flag = CreateEntity<MetalDetectorFlag>();
                if (flag == null)
                {
                    return false;
                }
                ...
                foreach (var loot in flag.LootLists)
                {
                    foreach (var item in loot.Items)
                    {
                        AddItem(item.Item, item.Min, item.Max, 0.5f, item.Condition ?? 1);
                    }
                }

                DestroyEntity(flag);

                config.Amount = new MinMax(1);
                config.LootType = LootType.Custom;
                config.Validate();
                return true;

                void AddItem(ItemDefinition def, int amount, int amountMax, float chance, float condition)
                {
                    config.DefaultCategory.Add(LootItem.Create(def.itemid, new MinMax(amount, amountMax), chance, new MinMaxFloat(condition)));
                }
            }
        }
        ...
        private class Crate : BaseLootable
        {
            ...
            public override bool LoadDefaultConfig(LootConfig config)
            {
                var entity = CreateEntity<BaseEntity>();

                var lootFill = entity.GetComponent<LootFill>();
                ...
                if (lootFill != null)
                {
                    ...
                    if (lootFill.LootSpawnSlots?.Length > 0)
                    {
                        foreach (var slot in lootFill.LootSpawnSlots)
                        {
                            LootConfigHelper.AddItems(config, slot.definition, slot.probability);
                        }
                    }
                    else if (lootFill.LootDefinition != null)
                    {
                        LootConfigHelper.AddItems(config, lootFill.LootDefinition);
                    }
                }
        ...
        private class Npc : BaseLootable
        {
            ...
            public override bool LoadDefaultConfig(LootConfig config)
            {
                ...
                {
                    var fsm = npc2.GetComponent<Scientist2FSM>();
                    var lootSpawnSlots = fsm?.dead?.GetField<State_Dead, LootContainer.LootSpawnSlot[]>("LootSpawnSlots");
        ...
        class PlayerUi
        {
            ...
            private void CustomItems()
            {
                                ...
                                : cui.CreateContainer((BOUNDS_X_L, 0.8f, 0.02f, 0.94f), UiColor.Transparent))
                {
                    ...
                    foreach (var item in customItems)
                    {
                        ...
                        {
                            EditItem(item);
                        } : null, giveItem: GiveToPlayer);

                        continue;

                        void GiveToPlayer(BasePlayer p)
                        {
                            var lootItem = LootItem.Create(item.ItemId, new MinMax(1));
                            lootItem.SkinId = item.SkinId;
                            lootItem.CustomName = item.CustomName;
                            lootItem.Text = item.Text;
                            LootManager.GiveItemToPlayer(lootItem, p);
                        }
                    }
                }
            ...
            private static void CreateItemPanel(Cui cui, in CuiAnchor anchor, int itemId, ulong skinId, string text, Action<BasePlayer> onClick,
                in MinMaxFloat condition = default, bool isBlueprint = false, int categoryId = -1, UiColor? backgroundOverride = null, int bulletCount = 0, Action<BasePlayer> giveItem = null)
            {
                ...
                using (cui.CreateContainer(anchor, background))
                {
                    ...
                    if (giveItem != null)
                    {
                        cui.AddButton((0.4f, 0.93f, 0.8f, 0.95f), "GIVE", giveItem, Style.ColorGreen, Style.TextItem, false);
                    }
                }
            }
        ...
        private void CmdLoottable(IPlayer iPlayer, string command, string[] args)
        {
            ...
            if (action == "add_new_items")
            {
                iPlayer.Reply("Adding new items ...");
                AddNewItemsToConfigs(out var result);
                iPlayer.Reply(result.ToSentence());
                return;
            }

            if (action == "list_new_items")
            {
                var newItems = state.ItemSave.NewItems?.Select(x => ItemManager.FindItemDefinition(x)?.shortname).ToSentence() ?? "no new items detected";
                iPlayer.Reply($"New items: {newItems}");
                return;
            }

            if (action == "dump_json")
            {
                iPlayer.Reply("Dumping loot table json ...");
                manager.DumpJsonConfigs();
                return;
            }
        ...
        private class TheManagerThatManagesEverything
        {
            ...
            public LootConfig GetConfig(ScientistNPC2 npc)
            {
                var id = lootableManager.GetConfigId(npc, true);
                return InternalGetConfig(id);
            }
            ...
            internal void DumpJsonConfigs()
            {
                foreach (var lootable in GetAllLootables())
                {
                    var config = vanillaConfigManager.GetConfig(lootable.Id, true, out _);
                    if (config != null)
                    {
                        VanillaConfigManager.WriteJsonConfig(config);
                    }
                }
            }
        }
        ...
        private class VanillaConfigManager : IConfigManager
        {
            ...
            public LootConfig GetConfig(ConfigId id, bool createIfNotExists, out bool justCreated)
            {
                ...
                if (Config.UseJsonLootConfigs)
                {
                    config = ReadJsonConfig(id);
                    if (config != null)
                    {
                        WriteDataFile(config, "loot", id.Key); // Update data file on successful load
                        return config;
                    }

                    LogError($"Failed to read config for {id}. Falling back to last version");
                }
            ...
            [CanBeNull]
            private static LootConfig ReadJsonConfig(ConfigId id)
            {
                var path = Path.Combine(Interface.Oxide.ConfigDirectory, "Loottable", $"{id.Key}.json");
                if (!File.Exists(path))
                {
                    return null;
                }

                JsonLootConfig jConfig;
                try
                {
                    var json = File.ReadAllText(path);
                    jConfig = JsonConvert.DeserializeObject<JsonLootConfig>(json);
                }
                catch (Exception ex)
                {
                    LogError($"Failed to load loot config {path}: {ex}");
                    return null;
                }

                var config = jConfig?.ToLootConfig(id);

                return config;
            }

            public static void WriteJsonConfig(LootConfig config)
            {
                var dir = Path.Combine(Interface.Oxide.ConfigDirectory, "Loottable");
                if (!Directory.Exists(dir))
                {
                    Directory.CreateDirectory(dir);
                }

                var path = Path.Combine(dir, $"{config.Id.Key}.json");
                var jConfig = JsonLootConfig.FromLootConfig(config);
                var json = JsonConvert.SerializeObject(jConfig, Formatting.Indented);
        ...
        private class JsonLootConfig
        {
            public bool Enabled { get; init; }
            public int ItemCountMin { get; init; }
            public int ItemCountMax { get; init; }
            public List<JsonLootItem> Items { get; init; }
            public Dictionary<int, JsonLootCategory> Categories { get; init; }


            [CanBeNull]
            public LootConfig ToLootConfig(ConfigId id)
            {
                var config = LootConfig.Create(id);

                foreach (var cat in Categories.Values)
                {
                    config.AddCategory(new LootCategory
                    {
                        Chance = cat.Chance,
                        Amount = new MinMax(cat.AmountMin, cat.AmountMax)
                    });
                }

                config.Amount = new MinMax(ItemCountMin, ItemCountMax);
                foreach (var itm in Items)
                {
                    var lootItem = itm.ToLootItem(out var categoryIdx);
                    if (lootItem == null)
                    {
                        continue;
                    }

                    if (categoryIdx == LootConfig.DEFAULT_CATEGORY_ID)
                    {
                        config.DefaultCategory.Add(lootItem);
                    }
                    else
                    {
                        var cat = config.GetCategory(categoryIdx);
                        if (cat == null)
                        {
                            LogError($"CONFIG ERROR! Can not add item {lootItem} to category {categoryIdx} - category does not exist");
                            return null;
                        }
                        cat.Items.Add(lootItem);
                    }
                }

                if (!config.SetEnabled(Enabled, out var error))
                {
                    LogError($"CONFIG ERROR! Failed to enable loot config {id.Key}: {error}");
                    return null;
                }
            ...
            public static JsonLootConfig FromLootConfig(LootConfig config)
            {
                var items = new List<JsonLootItem>();

                foreach (var item in config.GetAllItems(true))
                {
                    if (item.Item == null)
                    {
                        continue;
                    ...
                    var itm = JsonLootItem.FromLootItem(item.Item, item.Category);
                    items.Add(itm);
                }

                var categories = new Dictionary<int, JsonLootCategory>();

                foreach (var category in config.GetCategories())
                {
                    var c = JsonLootCategory.FromLootCategory(category);
                    categories.Add(category.Id, c);
                }

                return new JsonLootConfig()
                {
                    Enabled = config.Enabled,
                    ItemCountMin = config.Amount.Min,
                    ItemCountMax = config.Amount.Max,
                    Items = items,
        ...
        private class JsonLootItem
        {
            private static readonly List<JsonLootItem> EmptyItems = new(0);
            ...
            public float ConditionMin { get; init; }
            public float ConditionMax { get; init; }
            public int Category { get; init; }

            [CanBeNull]
            public List<JsonLootItem> AdditionalItems { get; init; }

            [CanBeNull]
            public LootItem ToLootItem(out int category, bool recursive = false)
            {
                category = 0;
                var itemDef = ItemManager.FindItemDefinition(ShortName);
                if (itemDef == null)
                {
                    LogError($"CONFIG ERROR! Item with shortname '{ShortName}' does not exist");
                    return null;
                }

                var amount = new MinMax(AmountMin, AmountMax);
                var condition = new MinMaxFloat(ConditionMin, ConditionMax);
                category = Mathf.Max(-1, Category);

                var item = LootItem.Create(itemDef.itemid, amount, SpawnChance, condition);
                item.SkinId = SkinId;
                if (!String.IsNullOrEmpty(CustomName))
                {
                    item.CustomName = CustomName;
                }
                if (!recursive && AdditionalItems != null && AdditionalItems.Count > 0)
                {
                    foreach (var additionalItem in AdditionalItems)
                    {
                        var a = additionalItem.ToLootItem(out _, true);
                        item.Extras ??= new List<LootItem>();
                        item.Extras.Add(a);
                    }
                }
                return item;
            ...
            public static JsonLootItem FromLootItem(LootItem item, int category)
            {
                return new JsonLootItem
                {
                    ShortName = item.ItemDefinition.shortname,
                    AmountMin = item.Amount.Min,
                    AmountMax = item.Amount.Max,
                    SpawnChance = category < 0 ? item.Chance : 1,
                    SkinId = item.SkinId,
                    CustomName = item.CustomName,
                    ConditionMin = item.Condition.Min,
                    ConditionMax = item.Condition.Max,
                    Category = LootConfig.DEFAULT_CATEGORY_ID,
                    AdditionalItems = item.Extras?.Select(x => FromLootItem(x, LootConfig.DEFAULT_CATEGORY_ID)).ToList() ?? EmptyItems
                };
            }
        }

        private class JsonLootCategory
        {
            public float Chance { get; init; }
            public int AmountMin { get; init; }
            public int AmountMax { get; init; }

            public static JsonLootCategory FromLootCategory(LootCategory category)
            {
                return new JsonLootCategory
                {
                    Chance = category.Chance,
                    AmountMin = category.Amount.Min,
                    AmountMax = category.Amount.Max
                };
            }
        }
        ...
        private class LootConfig
        {
            ...
            public LootCategory GetCategory(int index)
            {
                return Categories.GetValueOrDefault(index);
            }
            ...
            private void Validate([CanBeNull] out string error)
            {
                ...
                if (!Flags.HasFlag(LootConfigFlags.Quarry) && !Flags.HasFlag(LootConfigFlags.Excavator) && LootType == LootType.Custom && Amount.Max < 1)
                {
                    LogDebug($"Amount must be >0 in {Id}");
                    Enabled = false;
                    error = "amount_gt_0";
                }
            }
        ...
        public class Configuration
        {
            ...
            [JsonProperty("Use json loot configs from folder config/Loottable")]
            public bool UseJsonLootConfigs { get; set; }
            ...
            [JsonProperty("Smelting options")]
            public SmeltOptions SmeltingOptions { get; set; } = new()
            {
                AdjustProgressIndicator = true,
                MaxAdjustValue = 4,
            ...
            public class LootGeneratorOptions
            {
                ...
                public bool CountExtraItems { get; init; }
            ...
            public class StackSizeOptions
            {
                ...
                public bool IgnoreNames { get; init; }
                [JsonProperty("Allow stacking of items with different text")]
                public bool IgnoreText { get; init; }

                [JsonProperty("Allow stacking of damaged items with the same condition (vanilla only allows stacking when items have full condition)")]
                public bool IgnoreFullCondition { get; init; }
            }

            public class SmeltOptions
            {
                [JsonProperty("Adjust visual smelting progress indicator")]
                public bool AdjustProgressIndicator { get; init; }
                [JsonProperty("Adjust visual smelting progress indicator for smelting speeds less than X (Values larger than 10 not recommended, can cause lag on the CLIENT)")]
                public float MaxAdjustValue { get; init; }
                [JsonProperty("Show a warning when another plugin tries to override the smelting behavior")]
                public bool ShowHookWarnings { get; init; }
            }
        }
        ...
        private class SmeltingConfiguration
        {
            ...
            {
                var burnable = ItemManager.FindItemDefinition(-151838493)?.ItemModBurnable;
                if (burnable == null)
                {
            ...
            public void ToggleRecycler(Recycler recycler)
            {
                ...
                var think = (Action)AccessTools.Method(typeof(Recycler), "RecycleThink").CreateDelegate(typeof(Action), recycler);

                //recycler.CancelInvoke(recycler.RecycleThink);
                recycler.CancelInvoke(think);

                //recycler.lastFetchedEfficiency = recycler.IsSafezoneRecycler() ? RecyclerSafeEfficiency : RecyclerEfficiency;
                recycler.SetField("lastFetchedEfficiency", recycler.IsSafezoneRecycler() ? RecyclerSafeEfficiency : RecyclerEfficiency);

                Instance.NextTick(() =>
                {
                    //recycler.InvokeRepeating(recycler.RecycleThink, recyclerTickTime, recyclerTickTime);
                    recycler.InvokeRepeating(think, recyclerTickTime, recyclerTickTime);
                });
            }

            public float? GetSpeedMultiplier(BaseOven oven)
            {
                return oven.prefabID switch
                {
                    919097516 or 1057236622 => RefinerySpeed, // "small_refinery_static" or "refinery_small_deployed"
                    2931042549 or 2013224025 => SmallFurnaceSpeed, // "furnace" or "legacy_furnace"
                    1374462671 or 480290137 => LargeFurnaceSpeed, // "furnace.large" or "industrial.furnace.large"
                    4160694184 => CampfireSpeed, // "campfire"
                    2409469892 or 128449714 or 787806833 => BBQSpeed, // "bbq.deployed" or "bbq.static" or "bbq.static_hidden"
                    3808299817 or 3924463048 => ElectricFurnaceSpeed, // "electricfurnace.deployed" or "industrial_electric_furnace.deployed"
                    _ => null
                };
            }

            public static void Cook(BaseOven oven, float deltaTime, float multiplier)
            {
                if (oven.HasFlag(BaseEntity.Flags.Reserved8))
                {
                    return;
                }

                var burnable = oven.FindBurnable();
                if (Interface.CallHook("OnOvenCook", oven, burnable) != null)
                {
                    return;
                }

                if (burnable == null && !oven.CanRunWithNoFuel)
                {
                    oven.StopCooking();
                }
                else
                {
                    ...
                    {
                        if (item.position >= oven._inputSlotIndex && item.position < oven._inputSlotIndex + oven.inputSlots && !item.HasFlag(global::Item.Flag.Cooking))
                        {
                    ...
                    oven.IncreaseCookTime(deltaTime * oven.smeltSpeed * multiplier); // DO NOT USE GetSmeltingSpeed() here because it already includes the speed multiplier

                    var slot = oven.GetSlot(BaseEntity.Slot.FireMod);
                    if (slot)
                    {
                        slot.SendMessage(nameof(BaseOven.Cook), deltaTime, SendMessageOptions.DontRequireReceiver);
                    }

                    if (burnable != null)
                    {
                        var itemModBurnable = burnable.info.ItemModBurnable;
                        //burnable.fuel -= deltaTime * (oven.cookingTemperature / 200f);
                        var fuelToConsume = deltaTime * multiplier * (oven.cookingTemperature / 200f);

                        do
                        {
                            var consumeNow = Mathf.Min(fuelToConsume, burnable.fuel);
                            //LogDebug($"To consume {fuelToConsume} now {consumeNow}");
                            fuelToConsume -= consumeNow;
                            burnable.fuel -= consumeNow;
                            ...
                            {
                                LogDebug($"Consume fuel {fuelToConsume} {consumeNow}");
                                oven.ConsumeFuel(burnable, itemModBurnable);
                                ItemManager.DoRemoves();

                                burnable = oven.FindBurnable();
                                if (burnable == null)
                                {
                                    oven.StopCooking();
                                    break;
                    ...
                    oven.CallMethod<BaseOven>("OnCooked");
                    Interface.CallHook("OnOvenCooked", oven, burnable, slot);
                }
            }

            public static void Cook2(BaseOven oven, float deltaTime, float multiplier)
            {
                if (oven.HasFlag(BaseEntity.Flags.Reserved8))
                {
                    return;
                ...
                if (Interface.CallHook("OnOvenCook", oven, burnable) != null)
                {
                    if (Config.SmeltingOptions.ShowHookWarnings)
                    {
                        LogWarning("Another plugin is overriding the smelting behavior (OnOvenCook). Smelting might not work as expected. You can disable this warning in the config file.");
                    }
                    return;
                }
                ...
                if (burnable != null)
                {
                    ...
                    var fuelToConsume = deltaTime * multiplier * (oven.cookingTemperature / 200f);
                    fuelFractionConsumed = ConsumeFuel(oven, burnable, burnable.info.ItemModBurnable, fuelToConsume, ref turnOff);

                    // if (fuelFractionConsumed < 1)
                    // {
                    //     LogDebug($"Reduce cook time to {fuelFractionConsumed:F3} because not enough fuel");
                    // }
                }


                var cookTime = deltaTime * oven.smeltSpeed * multiplier; // DO NOT USE GetSmeltingSpeed() here because it already includes the speed multiplier
                oven.IncreaseCookTime(cookTime * fuelFractionConsumed);
                ...
                if (oven.HasFlag(BaseEntity.Flags.On) && turnOff)
                {
                    oven.StopCooking();
                }
            ...
            private static float ConsumeFuel(BaseOven oven, Item fuel, ItemModBurnable burnable, float fuelAmount, ref bool isFull)
            {
                if (Interface.CallHook("OnFuelConsume", oven, fuel, burnable) != null)
                {
                    if (Config.SmeltingOptions.ShowHookWarnings)
                    {
                        LogWarning("Another plugin is overriding the smelting behavior (OnFuelConsume). Smelting might not work as expected. You can disable this warning in the config file.");
                    }
                    return 1;
                }

                var fuelAvailable = burnable.fuelAmount * (fuel.amount - 1) + fuel.fuel;
                var fuelToConsume = Mathf.Min(fuelAvailable, fuelAmount);

                var fuelItemsToConsume = 0;
                ...
                if (fuelItemsToConsume >= fuel.amount)
                {
                    fuel.RemoveFromContainer(); // This is necessary because FindBurnable() can also find removed items in the oven
                    fuel.Remove();
                }
                else
                {
                    fuel.UseItem(fuelItemsToConsume);
                }

                var byproductCount = 0;
                if (oven.allowByproductCreation && burnable.byproductItem != null)
                {
                    for (int i = 0; i < fuelItemsToConsume; i++)
                    {
                        if (Random.Range(0f, 1f) > burnable.byproductChance) // This should be < but facepunch also does it like that so we have to keep it
                        {
                            byproductCount += burnable.byproductAmount * oven.GetCharcoalRate();
                        }
                    }
                }
                ...
                if (byproductCount > 0)
                {
                    foreach (var item in oven.inventory.itemList)
                    {
                        if (item.info == burnable.byproductItem && item.amount < item.info.stackable)
                        {
                            var add = Mathf.Min(item.info.stackable - item.amount, byproductCount);
                            byproductCount -= add;
                            item.amount += add;
                            item.MarkDirty();
                        }
                    ...
                    if (byproductCount > 0)
                    {
                        var byproduct = ItemManager.Create(burnable.byproductItem, byproductCount);
                        if (!byproduct.MoveToContainer(oven.inventory))
                        {
                            byproduct.Drop(oven.inventory.dropPosition, oven.inventory.dropVelocity);
                            //oven.OvenFull(); // Do not call this here because the current smelting tick is not yet done
                            isFull = true;
                        }
                    }
                }

                if (!fuel.IsRemoved()) // This doesn't make sense but its vanilla behavior
                {
                    Interface.CallHook("OnFuelConsumed", oven, fuel, burnable);
                }

                // Returns the actual consumed fraction of the fuel amount
                return Mathf.Clamp01(fuelToConsume / fuelAmount);
            }
        }
        ...
        class StackSizeController
        {
            ...
            {
                1036321299, 1223900335, -602717596, /* -3891563532, */ // Dog tags
        ...
        static class ItemHelper
        {
            ...
            private static readonly HashSet<int> signItems = new();

            public static void Init()
            {
                foreach (var def in ItemManager.GetItemDefinitions())
                {
                    var signMod = def.GetComponent<ItemModSign>();
                    if (signMod)
                    {
                        signItems.Add(def.itemid);
                    }
            ...
            public static bool IsSign(int itemId)
            {
                return signItems.Contains(itemId);
            }
...
throw;
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>()
            {
                ...
                ["amount_gt_0"] = "Total item amount must be greater than 0",
                ...
                ["crate_ptboat"] = "Patrol Boat Crate",
                ["crate_rhib"] = "RHIB Crate",
                ["mission_lootbox_basic"] = "Mission Loot Box",
                ["crate_satellite"] = "Satellite Crates",
                ...
                ["npc_oilrig_large"] = "Large Oil Rig Scientist",
                ...
                ["npc_deepsea"] = "Deep Sea Scientist",
                ["npc_ptboat"] = "Patrol Boat Scientist",
                ...
                ["metal_detector_flag"] = "Metal Detector Flag",
                ["metal_detector_flag_deepsea"] = "Deep Sea Metal Detector Flag",
...
namespace Oxide.Plugins.LoottableInterop
{
    internal static class LootableList
    {
        ...
        public static LootableDataList Get()
        {
            ...
            {
                MetalDetectorFlags = new MetalDetectorFlagLootableData[]
                {
                    new("metal_detector_flag", Url("metal_detector_flag.png"), "assets/prefabs/tools/metaldetector/metal_detector_flag.entity.prefab"),
                    new("metal_detector_flag_deepsea", Url("metal_detector_flag.png"), "assets/prefabs/tools/metaldetector/metal_detector_flag_deepsea.entity.prefab")
                },
                ...
                NpcData = new NpcData[]
                {
                    ...
                    new("npc_outbreak", Url("npc_outbreak.png"), null, "assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_outbreak.prefab"),
                    ...
                    new("npc_oilrig", Url("scientist_naval.png"), "assets/bundled/prefabs/autospawn/monument/offshore/oilrig_2.prefab",
                    ...
                    new("npc_oilrig_large", Url("scientist_naval.png"), "assets/bundled/prefabs/autospawn/monument/offshore/oilrig_1.prefab",
                        "assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_oilrig.prefab",
                        "assets/rust.ai/agents/npcplayer/humannpc/scientist/gen2/scientist2.prefab"
                    ),
                    ...
                    new("npc_deepsea", Url("scientist_naval.png"), null, "assets/rust.ai/agents/npcplayer/humannpc/scientist/gen2/scientist2.prefab"),
                    new ("npc_ptboat", null, null, "assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_ptboat.prefab", "assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_rhib.prefab"),
                ...
                CrateData = new CrateData[]
                {
                    ...
                    new("crate_ptboat", Url("rhib_storage_drybox.png"), "assets/content/vehicles/boats/ptboat/ptboat.deepsea.prefab"),
                    new("crate_rhib", Url("rhib_storage_drybox.png"), "assets/content/vehicles/boats/rhib/rhib.deepsea.prefab"),

                    // 65 Mission loot box
                    new("mission_lootbox_basic", Url("crate_tools0.png"), "assets/prefabs/missions/entities/missionlootbox_basic.prefab"),

                    // 66 Satellite crates
                    new("crate_satellite", Url("crate_satellite.png"), "assets/prefabs/satellitecrash/crates/satellite_crate_1.entity.prefab",
                        "assets/prefabs/satellitecrash/crates/satellite_crate_2.entity.prefab", "assets/prefabs/satellitecrash/crates/satellite_crate_3.entity.prefab")
                }
            };

            list.AddTab("crates", "crates_normal", list.CrateData, 0, 3);
            list.AddTab("crates", "crates_special", list.CrateData, 61, 61);
            list.AddTab("crates", "crates_special", list.CrateData, 15, 19);
            list.AddTab("crates", "crates_special", list.CrateData, 66, 66);
            ...
            list.AddTab("crates", "crates_misc", list.CrateData, 62, 65);

            list.AddTab("npcs", "land", list.NpcData, 0, 14);
            list.AddTab("npcs", "offshore", list.NpcData, 15, 21);
            list.AddTab("npcs", "seasonal", list.NpcData, 22, list.NpcData.Length - 1);
            ...
            list.AddTab("gathering", "metal_detector", list.MetalDetectorFlags, 0, 1);
    ...
    {
        public MetalDetectorFlagLootableData[] MetalDetectorFlags { get; init; } = Array.Empty<MetalDetectorFlagLootableData>();
    ...
    internal class MetalDetectorFlagLootableData : BaseLootableData
    {
        public string PrefabName { get; init; }

        public MetalDetectorFlagLootableData(string key, string imageUrl, string prefabName)
            : base(key, imageUrl)
        {
