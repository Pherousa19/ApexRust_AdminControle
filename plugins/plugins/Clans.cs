    ...
    public class Clans : CovalencePlugin
    {
        ...
        private void LoadData()
        {
            ...
            else
            {
                ...
                // Rebuild the in-memory player -> clan tag lookup every time data is
                // (re)loaded. playerLookup is [JsonIgnore] and therefore never
                // persisted, so without this it is only populated once, during
                // OnServerInitialized. If this plugin is ever reloaded mid-session
                // (oxide.reload Clans, hot-reload, etc.) without a full server
                // restart, OnServerInitialized will not fire again and
                // FindClanByID/GetClanOf would silently return null for everyone
                // from that point on.
                storedData.RebuildPlayerLookup();
            }
        }
        ...
        private void RestoreClanData(Dictionary<string, OldClan> clanData)
        {
            ...
            // RestoreClanData builds ClanMembers directly rather than going through
            // RegisterPlayer for every member, so make sure the lookup reflects the
            // restored clans before anything queries it.
            storedData.RebuildPlayerLookup();
        ...
        public class StoredData
        {
            ...
            // Rebuilds playerLookup from scratch based on the current clans/members.
            // Needed because playerLookup is [JsonIgnore] and never persisted -
            // call this any time storedData is (re)constructed from disk.
            internal void RebuildPlayerLookup()
            {
                playerLookup.Clear();

                foreach (KeyValuePair<string, Clan> kvp in clans)
                {
                    foreach (string memberId in kvp.Value.ClanMembers.Keys)
                        playerLookup[memberId] = kvp.Key;
                }
            }
