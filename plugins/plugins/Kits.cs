...
{
    // Heavily modified version of the original "Kits" plugin by K1lly0u.
    // Most of the UI, item picker/search, and kit logic below has been substantially
    // rewritten from the original, but the base concept and structure are his.
    [Info("Kits", "Noobless Gaming", "1.0.0"), Description("Create kits containing items that players can redeem, with an in-game item search builder, permissions, cooldowns and costs.")]
    class Kits : RustPlugin
    {
        ...
        // Item picker state (lets admins add items to a kit by search instead of only
        // copying whatever is currently in their own inventory).
        private readonly Hash<ulong, bool> _pickerOpen = new Hash<ulong, bool>();
        private readonly Hash<ulong, string> _pickerQuery = new Hash<ulong, string>();
        private readonly Hash<ulong, string> _pickerSelected = new Hash<ulong, string>();
        private readonly Hash<ulong, int> _pickerAmount = new Hash<ulong, int>();
        ...
        {
            if (!Configuration.MailNotifyOnConnect || !player)
                return;

            int pending = mailData.CountPending(player.userID);
            if (pending > 0)
                player.ChatMessage(string.Format(Message("Mail.Notify", player.userID), pending));
        }
        ...
        private void Unload()
        {
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
                ...
                CuiHelper.DestroyUi(player, UI_PICKER);
            }

            Configuration = null;
        }

        private void ClearPickerState(ulong userId)
        {
            _pickerOpen[userId] = false;
            _pickerQuery[userId] = string.Empty;
            _pickerSelected[userId] = string.Empty;
            _pickerAmount[userId] = 0;
            _pickerSkin[userId] = 0UL;
            _pickerSlot[userId] = string.Empty;
            _pickerPage[userId] = 0;
        }
        #endregion
        ...
        private void ClaimMail(BasePlayer player)
        {
            List<MailData.PendingKit> pending = mailData.GetPending(player.userID);
            if (pending == null || pending.Count == 0)
            {
                player.ChatMessage(Message("Mail.None", player.userID));
                return;
            }

            int delivered = 0;
            foreach (MailData.PendingKit entry in pending.ToArray())
            {
                if (!kitData.Find(entry.KitName, out KitData.Kit kit))
                {
                    mailData.Remove(player.userID, entry);
                    continue;
                }
            ...
            {
                SaveMailData();
                player.ChatMessage(string.Format(Message("Mail.Claimed", player.userID), delivered));
            }

            if (mailData.HasPending(player.userID))
                player.ChatMessage(Message("Mail.MakeRoom", player.userID));
        }
        #endregion
        ...
        // Permission-restricted kits are intentionally visible to everyone.
        // Visibility is separate from redemption: CanClaimKit() remains the authoritative
        // server-side permission check, so seeing a kit never grants access to it.
        ...
        private const string UI_PICKER = "kits.picker";
        ...
        private void CreateKitLayout(BasePlayer player, CuiElementContainer container, KitData.Kit kit, bool isEditor = false)
        {
            ...
            CreateInventoryItems(player, container, MainAlign, kit.MainItems, 24, "main", isEditor);
            ...
            CreateInventoryItems(player, container, WearAlign, kit.WearItems, 8, "wear", isEditor);
            ...
            CreateInventoryItems(player, container, BeltAlign, kit.BeltItems, 6, "belt", isEditor);            
        }

        #region Item Layout Helpers
        // isEditor: when true (admin building/editing a kit) filled slots become clickable
        // and remove the item from the kit - lets an admin curate a kit without needing to
        // hold every item themselves first.
        private void CreateInventoryItems(BasePlayer player, CuiElementContainer container, GridAlignment alignment, ItemData[] items, int capacity, string slot = "main", bool isEditor = false)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                ...
                if (isEditor)
                    UI.Button(container, UI_MENU, "0 0 0 0", string.Empty, 0, position, $"kits.removeitem {slot} {itemData.Position}");
            }
        }
        #endregion
        #endregion
        ...
        private void OpenKitsEditor(BasePlayer player, bool overwrite = false)
        {
            ...
            // Kit Items
            CreateKitLayout(player, container, kit, true);
            ...
            // Item Management            
            UI.Button(container, UI_MENU, Configuration.Menu.Color3.Get, Message("UI.ClearItems", player.userID), 14, new UI4(0.505f, 0.005f, 0.65f, 0.045f), $"kits.clearitems {overwrite}");
            UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.CopyInv", player.userID), 14, new UI4(0.655f, 0.005f, 0.8f, 0.045f), $"kits.copyinv {overwrite}");
            UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.AddItem", player.userID), 14, new UI4(0.805f, 0.005f, 0.995f, 0.045f), "kits.picker open");

            CuiHelper.DestroyUi(player, UI_MENU);
            CuiHelper.AddUi(player, container);

            if (_pickerOpen[player.userID])
        ...
        #region Item Picker
        private const int PICKER_RESULTS = 8;

        private void OpenItemPicker(BasePlayer player)
        {
            ulong userId = player.userID;

            CuiElementContainer container = UI.Container(UI_PICKER, "0 0 0 0.92", new UI4(0.3f, 0.2f, 0.7f, 0.8f), true, "Hud");

            UI.Panel(container, UI_PICKER, Configuration.Menu.Panel.Get, new UI4(0.02f, 0.92f, 0.98f, 0.98f));
            UI.Label(container, UI_PICKER, Message("UI.AddItem", userId), 18, new UI4(0.03f, 0.92f, 0.9f, 0.98f), TextAnchor.MiddleLeft);
            UI.Button(container, UI_PICKER, Configuration.Menu.Color3.Get, "<b>×</b>", 18, new UI4(0.92f, 0.925f, 0.975f, 0.975f), "kits.picker close");

            string query = _pickerQuery[userId] ?? string.Empty;

            // Search box
            UI.Panel(container, UI_PICKER, ICON_BACKGROUND_COLOR, new UI4(0.03f, 0.85f, 0.97f, 0.9f));
            if (string.IsNullOrEmpty(query))
                UI.Input(container, UI_PICKER, string.Empty, 14, "kits.itemsearch ", new UI4(0.04f, 0.85f, 0.96f, 0.9f));
            else
            {
                UI.Label(container, UI_PICKER, query, 14, new UI4(0.04f, 0.85f, 0.9f, 0.9f), TextAnchor.MiddleLeft);
                UI.Button(container, UI_PICKER, Configuration.Menu.Color3.Get, "X", 12, new UI4(0.9f, 0.86f, 0.96f, 0.89f), "kits.itemsearch ");
            }

            // Results
            List<ItemDefinition> allResults = SearchItems(query);
            int totalPages = Mathf.Max(1, Mathf.CeilToInt(allResults.Count / (float)PICKER_RESULTS));
            int page = Mathf.Clamp(_pickerPage[userId], 0, totalPages - 1);
            _pickerPage[userId] = page;
            List<ItemDefinition> results = allResults.Skip(page * PICKER_RESULTS).Take(PICKER_RESULTS).ToList();

            // Pagination bar - sits in the gap between the search box and the first result row
            if (allResults.Count > PICKER_RESULTS)
            {
                UI.Label(container, UI_PICKER, $"{allResults.Count} results - page {page + 1}/{totalPages}", 11, new UI4(0.2f, 0.825f, 0.65f, 0.845f), TextAnchor.MiddleLeft);

                if (page > 0)
                    UI.Button(container, UI_PICKER, Configuration.Menu.Color3.Get, "<", 12, new UI4(0.03f, 0.825f, 0.1f, 0.845f), "kits.itempage prev");

                if (page < totalPages - 1)
                    UI.Button(container, UI_PICKER, Configuration.Menu.Color3.Get, ">", 12, new UI4(0.9f, 0.825f, 0.97f, 0.845f), "kits.itempage next");
            }

            for (int i = 0; i < results.Count; i++)
            {
                ItemDefinition def = results[i];
                float yMax = 0.82f - (i * 0.055f);
                float yMin = yMax - 0.05f;

                string selected = _pickerSelected[userId];
                string rowColor = !string.IsNullOrEmpty(selected) && string.Equals(selected, def.shortname, StringComparison.OrdinalIgnoreCase)
                    ? Configuration.Menu.Color2.Get
                    : ICON_BACKGROUND_COLOR;

                UI.Panel(container, UI_PICKER, rowColor, new UI4(0.03f, yMin, 0.97f, yMax));
                UI.Image(container, UI_PICKER, def.itemid, 0UL, new UI4(0.035f, yMin + 0.003f, 0.079f, yMax - 0.003f));
                UI.Label(container, UI_PICKER, def.displayName.english, 12, new UI4(0.09f, yMin, 0.7f, yMax), TextAnchor.MiddleLeft);
                UI.Button(container, UI_PICKER, "0 0 0 0", string.Empty, 0, new UI4(0.03f, yMin, 0.97f, yMax), $"kits.selectitem {def.shortname}");
            }

            if (results.Count == 0)
                UI.Label(container, UI_PICKER, Message("UI.NoResults", userId), 12, new UI4(0.03f, 0.75f, 0.97f, 0.8f), TextAnchor.MiddleLeft);

            // Selected item options
            string selectedShortname = _pickerSelected[userId];
            if (!string.IsNullOrEmpty(selectedShortname))
            {
                ItemDefinition selectedDef = ItemManager.FindItemDefinition(selectedShortname);
                if (selectedDef != null)
                {
                    UI.Panel(container, UI_PICKER, Configuration.Menu.Color4.Get, new UI4(0.03f, 0.31f, 0.97f, 0.35f));
                    UI.Label(container, UI_PICKER, selectedDef.displayName.english, 14, new UI4(0.04f, 0.31f, 0.97f, 0.35f), TextAnchor.MiddleLeft);

                    int amount = _pickerAmount[userId] <= 0 ? 1 : _pickerAmount[userId];
                    ulong skin = _pickerSkin[userId];
                    string slot = string.IsNullOrEmpty(_pickerSlot[userId]) ? DefaultSlotFor(selectedDef) : _pickerSlot[userId];

                    UI.Label(container, UI_PICKER, Message("UI.Amount", userId), 12, new UI4(0.03f, 0.25f, 0.2f, 0.29f), TextAnchor.MiddleLeft);
                    UI.Panel(container, UI_PICKER, ICON_BACKGROUND_COLOR, new UI4(0.2f, 0.25f, 0.4f, 0.29f));
                    UI.Input(container, UI_PICKER, amount.ToString(), 12, "kits.itemamount ", new UI4(0.205f, 0.25f, 0.395f, 0.29f));

                    UI.Label(container, UI_PICKER, Message("UI.SkinId", userId), 12, new UI4(0.42f, 0.25f, 0.55f, 0.29f), TextAnchor.MiddleLeft);
                    UI.Panel(container, UI_PICKER, ICON_BACKGROUND_COLOR, new UI4(0.55f, 0.25f, 0.8f, 0.29f));
                    UI.Input(container, UI_PICKER, skin == 0 ? string.Empty : skin.ToString(), 12, "kits.itemskin ", new UI4(0.555f, 0.25f, 0.795f, 0.29f));

                    UI.Label(container, UI_PICKER, Message("UI.Slot", userId), 12, new UI4(0.03f, 0.19f, 0.2f, 0.23f), TextAnchor.MiddleLeft);
                    AddSlotToggle(container, "main", Message("UI.MainItems", userId), slot, 0.2f);
                    AddSlotToggle(container, "wear", Message("UI.WearItems", userId), slot, 0.4f);
                    AddSlotToggle(container, "belt", Message("UI.BeltItems", userId), slot, 0.6f);

                    UI.Button(container, UI_PICKER, Configuration.Menu.Color2.Get, Message("UI.AddToKit", userId), 14, new UI4(0.03f, 0.12f, 0.97f, 0.17f), "kits.additem");
                }
            }
        ...
        private void AddSlotToggle(CuiElementContainer container, string slot, string label, string current, float xOffset)
        {
            bool isActive = slot == current;
            UI.Panel(container, UI_PICKER, isActive ? Configuration.Menu.Color2.Get : ICON_BACKGROUND_COLOR, new UI4(xOffset, 0.19f, xOffset + 0.18f, 0.23f));
            UI.Label(container, UI_PICKER, label, 11, new UI4(xOffset, 0.19f, xOffset + 0.18f, 0.23f), TextAnchor.MiddleCenter);
            UI.Button(container, UI_PICKER, "0 0 0 0", string.Empty, 0, new UI4(xOffset, 0.19f, xOffset + 0.18f, 0.23f), $"kits.itemslot {slot}");
        }

        private static string DefaultSlotFor(ItemDefinition def) => def.GetComponent<ItemModWearable>() != null ? "wear" : "main";

        private List<ItemDefinition> SearchItems(string query)
        {
            IEnumerable<ItemDefinition> source = ItemManager.itemList.Where(d => !d.shortname.Equals(BLUEPRINT_BASE));

            if (!string.IsNullOrEmpty(query))
            {
                source = source.Where(d =>
                    d.shortname.IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0 ||
                    d.displayName.english.IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0);
            }

            return source.OrderBy(d => d.displayName.english).ToList();
        }
        #endregion
        ...
        private void ccmdKitsClose(ConsoleSystem.Arg arg)
        {
            ...
            ClearPickerState(player.userID);
        ...
        [ConsoleCommand("kits.picker")]
        private void ccmdPicker(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player)
                return;
            ...
            _pickerOpen[player.userID] = true;
            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.itemsearch")]
        private void ccmdItemSearch(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player || !_pickerOpen[player.userID])
                return;

            _pickerQuery[player.userID] = arg.GetString(0) ?? string.Empty;
            _pickerPage[player.userID] = 0; // any new search starts back at page 1
            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.itempage")]
        private void ccmdItemPage(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player || !_pickerOpen[player.userID])
                return;
            ...
            int current = _pickerPage[player.userID];

            if (direction == "next")
                _pickerPage[player.userID] = current + 1;
            else if (direction == "prev")
                _pickerPage[player.userID] = Mathf.Max(0, current - 1);

            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.selectitem")]
        private void ccmdSelectItem(ConsoleSystem.Arg arg)
        {
            ...
            _pickerSelected[player.userID] = shortname;
            _pickerAmount[player.userID] = 1;
            _pickerSkin[player.userID] = 0UL;
            _pickerSlot[player.userID] = DefaultSlotFor(definition);

            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.itemamount")]
        private void ccmdItemAmount(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player || !_pickerOpen[player.userID])
                return;

            if (int.TryParse(arg.GetString(0), out int amount))
                _pickerAmount[player.userID] = Mathf.Max(1, amount);

            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.itemskin")]
        private void ccmdItemSkin(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player || !_pickerOpen[player.userID])
                return;

            if (ulong.TryParse(arg.GetString(0), out ulong skin))
                _pickerSkin[player.userID] = skin;
            else if (string.IsNullOrEmpty(arg.GetString(0)))
                _pickerSkin[player.userID] = 0UL;

            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.itemslot")]
        private void ccmdItemSlot(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player || !_pickerOpen[player.userID])
                return;

            string slot = arg.GetString(0);
            if (slot == "main" || slot == "wear" || slot == "belt")
                _pickerSlot[player.userID] = slot;

            OpenItemPicker(player);
        }

        [ConsoleCommand("kits.additem")]
        private void ccmdAddItem(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player)
                return;
            ...
            string shortname = _pickerSelected[player.userID];
            ...
            int amount = _pickerAmount[player.userID] <= 0 ? 1 : _pickerAmount[player.userID];
            ulong skin = _pickerSkin[player.userID];
            string slot = string.IsNullOrEmpty(_pickerSlot[player.userID]) ? DefaultSlotFor(definition) : _pickerSlot[player.userID];

            if (kit.TryAddItem(shortname, definition.displayName.english, amount, skin, slot, out string error))
            {
                OpenKitsEditor(player);
            }
            else if (error == "full")
                CreateMenuPopup(player, Message("Picker.Full", player.userID));
        }

        [ConsoleCommand("kits.removeitem")]
        private void ccmdRemoveItem(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (!player)
                return;
            ...
            if (kit.TryRemoveItem(slot, position))
                OpenKitsEditor(player);
        }
        ...
        private void cmdKit(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "mail":
                    if (args.Length >= 2 && args[1].Equals("claim", StringComparison.OrdinalIgnoreCase))
                    {
                        ClaimMail(player);
                        return;
                    }

                    List<MailData.PendingKit> pendingMail = mailData.GetPending(player.userID);
                    if (pendingMail == null || pendingMail.Count == 0)
                    {
                        player.ChatMessage(Message("Mail.None", player.userID));
                        return;
                    }

                    player.ChatMessage(string.Format(Message("Mail.Pending", player.userID), pendingMail.Count));
                    foreach (MailData.PendingKit entry in pendingMail)
                        player.ChatMessage($" - {entry.KitName}");
                    player.ChatMessage(Message("Mail.HowToClaim", player.userID));
                    return;
        ...
        private void ccmdKit(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args == null || arg.Args.Length == 0)
            {
                ...
                SendReply(arg, "kit mailgive <steamid> <kitname> - Give a kit now if online, otherwise queue it as mail");
            ...
            {               
                ...
                case "mailgive":
                    ...
                    if (!ulong.TryParse(arg.GetString(1), out ulong mailUserId))
                    {
                        SendReply(arg, $"mailgive:error:invalid_steamid:{arg.GetString(1)}");
                        return;
                    }

                    if (!kitData.Find(arg.GetString(2), out KitData.Kit mailKit))
                    {
                        SendReply(arg, $"mailgive:error:no_such_kit:{arg.GetString(2)}");
                        return;
                    }

                    BasePlayer mailTarget = FindPlayer(arg.GetString(1));
                    if (mailTarget != null && mailTarget.IsConnected && mailKit.HasSpaceForItems(mailTarget))
                    {
                        GiveKit(mailTarget, mailKit);
                        mailTarget.ChatMessage(string.Format(Message("Notification.MailKitReceived", mailTarget.userID), mailKit.Name));
                        SendReply(arg, $"mailgive:delivered:{mailUserId}:{mailKit.Name}");
                        return;
                    }

                    if (!Configuration.MailDelivery)
                    {
                        SendReply(arg, $"mailgive:error:player_offline:{mailUserId}");
                        return;
                    }

                    mailData.Add(mailUserId, mailKit.Name, "store");
                    SaveMailData();
                    if (mailTarget != null && mailTarget.IsConnected)
                        mailTarget.ChatMessage(string.Format(Message("Notification.MailQueuedFullInventory", mailTarget.userID), mailKit.Name));

                    SendReply(arg, $"mailgive:queued:{mailUserId}:{mailKit.Name}");
                    return;
        ...
        private class ConfigData
        {
            ...
            [JsonProperty(PropertyName = "Show kits with permissions assigned to players without the permission (legacy - permission kits are always visible)")]
            ...
            [JsonProperty(PropertyName = "Mail delivery - queue kits for offline players (via 'kit mailgive') instead of dropping them")]
            public bool MailDelivery { get; set; } = true;
        ...
        private void LoadData()
        {
            ...
            maildata = Interface.Oxide.DataFileSystem.GetFile("Kits/mail_data");

            kitData = kitdata.ReadObject<KitData>();
            playerData = playerdata.ReadObject<PlayerData>();
            mailData = maildata.ReadObject<MailData>();
        ...
        private class KitData
        {
            ...
            public class Kit
            {
                ...
                // Adds an item that the admin does not need to be holding - built straight
                // from an ItemDefinition instead of snapshotting a live Item.
                internal bool TryAddItem(string shortname, string displayName, int amount, ulong skin, string slot, out string error)
                {
                    error = null;

                    int capacity = GetCapacity(slot);
                    ItemData[] target = GetArray(slot);

                    int position = GetNextFreePosition(target, capacity);
                    if (position == -1)
                    {
                        error = "full";
                        return false;
                    ...
                    ItemData[] updated = new ItemData[target.Length + 1];
                    Array.Copy(target, updated, target.Length);
                    updated[target.Length] = new ItemData
                    {
                        Shortname = shortname,
                    ...
                    SetArray(slot, updated);
                    return true;
                }

                internal bool TryRemoveItem(string slot, int position)
                {
                    ItemData[] target = GetArray(slot);
                    ItemData[] updated = target.Where(x => x.Position != position).ToArray();
                ...
                private ItemData[] GetArray(string slot)
                {
                    switch (slot)
                    {
                        case "wear": return WearItems;
                        case "belt": return BeltItems;
                        default: return MainItems;
                    }
                }

                private void SetArray(string slot, ItemData[] value)
                {
                    switch (slot)
                    {
                        case "wear": WearItems = value; break;
                        case "belt": BeltItems = value; break;
                        default: MainItems = value; break;
                    }
                }

                private static int GetCapacity(string slot)
                {
                    switch (slot)
                    {
                        case "wear": return 8;
                ...
                private static int GetNextFreePosition(ItemData[] items, int capacity)
                {
        ...
        {
            [JsonProperty]
            private Hash<ulong, List<PendingKit>> _mail = new Hash<ulong, List<PendingKit>>();

            internal bool IsValid => _mail != null;

            internal List<PendingKit> GetPending(ulong userId) => _mail.TryGetValue(userId, out List<PendingKit> list) ? list : null;

            internal bool HasPending(ulong userId) => _mail.TryGetValue(userId, out List<PendingKit> list) && list.Count > 0;

            internal int CountPending(ulong userId) => _mail.TryGetValue(userId, out List<PendingKit> list) ? list.Count : 0;

            internal void Add(ulong userId, string kitName, string source)
            {
                if (!_mail.TryGetValue(userId, out List<PendingKit> list))
                    list = _mail[userId] = new List<PendingKit>();

                list.Add(new PendingKit { KitName = kitName, QueuedAt = CurrentTime, Source = source });
            }

            internal void Remove(ulong userId, PendingKit entry)
            {
                if (!_mail.TryGetValue(userId, out List<PendingKit> list))
                    return;
        ...
        private readonly Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            ["Notification.MailKitReceived"] = "You have received the kit: <color=#ce422b>{0}</color> (web store purchase)",
            ["Notification.MailQueuedFullInventory"] = "A kit (<color=#ce422b>{0}</color>) from your web store purchase is waiting - free up inventory space and type /kit mail claim",
            ["Mail.None"] = "You have no kits waiting to be claimed",
            ["Mail.Pending"] = "You have {0} kit(s) waiting to be claimed:",
            ["Mail.HowToClaim"] = "Type <color=#ce422b>/kit mail claim</color> to collect them",
            ["Mail.Claimed"] = "You have claimed {0} kit(s) from your mail",
            ["Mail.MakeRoom"] = "You still have kits waiting - free up inventory space and run /kit mail claim again",
            ["Mail.Notify"] = "You have {0} kit(s) waiting from a web store purchase - type <color=#ce422b>/kit mail</color> to view them",
            ...
            ["Chat.Help.12"] = "<color=#ce422b>/kit mail</color> - View and claim kits bought on the web store while offline",
            ...
            ["UI.AddItem"] = "Add Item",
            ["UI.NoResults"] = "No items match your search",
            ["UI.Amount"] = "Amount",
            ["UI.SkinId"] = "Skin ID",
            ["UI.Slot"] = "Slot",
            ["UI.AddToKit"] = "Add To Kit",
            ["Picker.Full"] = "That slot type is full - remove an item first",
            ["UI.CreateNew"] = "Create New",
            ["UI.EditKit"] = "Edit Kit",
            ["UI.NeedsPermission"] = "🔒 Needs Permission",
